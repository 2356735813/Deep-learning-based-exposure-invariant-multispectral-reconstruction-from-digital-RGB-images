...
class Illumination_Estimator(nn.Module):
    ...
    def forward(self, img):
        img_first_image = img[0]
        torch.save(img_first_image, 'img_first_image.pt')
        mean_c = img.mean(dim=1).unsqueeze(1)  # 照明先验
        mean_c_first_image = mean_c[0]
        torch.save(mean_c_first_image, 'mean_c_first_image.pt')
        ...
        illu_fea_first_image = illu_fea[0]
        torch.save(illu_fea_first_image, 'illu_fea_first_image.pt')
        # 最后的卷积得到照明映射
        illu_map = self.conv2(illu_fea)
        illu_map_first_image = illu_map[0]
        torch.save(illu_map_first_image, 'illu_map_first_image.pt')
...
class RetinexFormer_Single_Stage(nn.Module):
    def __init__(self, in_channels=3, out_channels=31, n_feat=31, level=2, num_blocks=[1, 1, 1]):
        ...
        self.denoiser = Denoiser(in_dim=31,out_dim=out_channels,dim=n_feat,level=level,num_blocks=num_blocks)
